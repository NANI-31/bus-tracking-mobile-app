...
import { getBuses, updateBusLocation } from "../slices/collegeAdminSlice";
...
const LiveTracking = () => {
  ...
  useEffect(() => {
    ...
    if (userToken && userInfo?.collegeId) {
      ...
      socket.emit("join_college", userInfo.collegeId);
        ...
        <div className="w-80 bg-white rounded-2xl border border-gray-200 shadow-sm flex flex-col overflow-hidden">
          <div className="p-4 border-b border-gray-100 bg-gray-50/50">
            <h2 className="font-bold text-gray-800 flex items-center text-sm">
              ...
              Active Fleet ({buses.filter((b) => b.lastLocation).length})
            </h2>
          </div>
          ...
          {selectedBus && (
            <div className="p-4 bg-blue-600 text-white rounded-t-2xl">
              ...
              <div className="space-y-2">
                ...
                <button className="w-full mt-2 bg-white text-blue-600 py-2 rounded-xl text-xs font-bold hover:bg-blue-50 transition-colors">
                  View Driver Profile
                </button>
              </div>
            </div>
