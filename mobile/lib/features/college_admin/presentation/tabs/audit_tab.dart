...
class _AuditTabState extends ConsumerState<AuditTab> {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      _socketSub = ref.read(socketServiceProvider).newAuditLogStream.listen((data) {
        if (mounted && !hasActiveFilters) {
          ...
          if (newLog.collegeId == collegeId) {
            ref.read(collegeAdminServiceProvider.notifier).addLiveLog(newLog);
          }
        }
      });
    });
  ...
  void _scrollListener() {
    if (_scrollController.position.pixels >=
      ...
      final state = ref.read(collegeAdminServiceProvider).valueOrNull;
  ...
  void _fetchLogs({bool isLoadMore = false}) async {
    ...
    if (collegeId.isNotEmpty) {
      await ref.read(collegeAdminServiceProvider.notifier).fetchAuditLogs(
            collegeId: collegeId,
            actions: actionsList,
  ...
  Widget _buildActivityChart(List<AuditLogModel> logs, bool isDark) {
    ...
    final Color lineAccentColor = isDark ? const Color(0xFF00C6E6) : const Color(0xFF0097B2);
  ...
  Widget build(BuildContext context) {
    final dashboardAsync = ref.watch(collegeAdminServiceProvider);
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final activeThemeColor = isDark ? const Color(0xFF00C6E6) : const Color(0xFF0097B2);
                      ...
                      : RefreshIndicator(
                          ...
                          child: ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return _buildAuditLogCard(log, isDark, activeThemeColor);
                            },
                          ),
                        ),
            ),
  ...
  Widget _buildAuditLogCard(AuditLogModel log, bool isDark, Color activeThemeColor) {
                      ...
                      onTap: isNavigable
                          ? () {
                              ...
                              if (resLower == 'bus') {
                                ...
                                context.push('/coordinator/edit-bus/$busNumber');
                              } else if (resLower == 'driver') {
                                context.push('/coordinator/edit-driver/${log.resourceId}');
                              } else if (resLower == 'schedule') {
                                context.push('/coordinator/schedule');
                              }
                            }
                          : null,
...
// Split Pane Categories Bottom Sheet (simplified for Action & Resource only)
...
class _AuditFilterBottomSheetState extends State<_AuditFilterBottomSheet> {
  int _activeTab = 0; // 0 = Action, 1 = Resource
          ...
          Expanded(
            child: Row(
              children: [
                ...
                Expanded(
                  child: _activeTab == 0
                      ? _buildActionOptionsList(isDark, activeThemeColor)
                      : _buildResourceOptionsList(isDark, activeThemeColor),
                ),
              ],
            ),
          ),
  ...
  Widget _buildActionOptionsList(bool isDark, Color activeColor) {
  ...
  Widget _buildResourceOptionsList(bool isDark, Color activeColor) {
    return ListView.builder(
      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 8),
...
// Log Metadata Diff details Bottom Sheet (Monospace style)
      ...
      child: Column(
        ...
        children: [
          Padding(
            ...
            child: Row(
              children: [
                Icon(Icons.difference_outlined, size: 14, color: lineAccentColor),
                const SizedBox(width: 6),
                Text(
                  title,
                  style: TextStyle(
