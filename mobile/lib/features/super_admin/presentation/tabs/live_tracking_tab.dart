...
class _LiveTrackingTabState extends ConsumerState<LiveTrackingTab> {
  ...
  void _showCollegeSearchSheet(BuildContext context, List<CollegeModel> colleges, bool isDark) {
        ...
        return StatefulBuilder(
          builder: (context, setModalState) {
            final filteredColleges = colleges.where((c) {
              return c.name.toLowerCase().contains(_collegeSearchTerm.toLowerCase());
            }).toList();
                  ...
                  Expanded(
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        if (index == 0) {
                          final isSelected = _selectedCollegeId == 'all';
                          return ListTile(
                            title: const Text('All Colleges', style: TextStyle(fontSize: 13)),
                            trailing: isSelected ? const Icon(Icons.check, color: Color(0xFF1E90FF)) : null,
                            onTap: () {
                              setState(() {
                                _selectedCollegeId = 'all';
                                _selectedBus = null;
                              });
                        ...
                        final college = filteredColleges[index - 1];
                        final isSelected = _selectedCollegeId == college.id;

                        return ListTile(
                          title: Text(college.name, style: const TextStyle(fontSize: 13)),
                          trailing: isSelected ? const Icon(Icons.check, color: Color(0xFF1E90FF)) : null,
                          onTap: () {
                            setState(() {
        ...
        return locationsAsync.when(
          data: (locations) {
            ...
            final filteredBuses = buses.where((b) {
              final matchCollege = _selectedCollegeId == 'all' || b.collegeId == _selectedCollegeId;
            ...
            final stateKey = '${_zoom}_${_selectedCollegeId}_${_searchTerm}_${filteredBuses.map((b) {
                ...
                Padding(
                  ...
                  child: Row(
                    children: [
                      Expanded(
                        child: InkWell(
                          onTap: () => _showCollegeSearchSheet(context, colleges, isDark),
                          borderRadius: BorderRadius.circular(12),
                          child: Container(
                              ...
                              children: [
                                Expanded(
                                  child: Text(
                                    _selectedCollegeId == 'all'
                                        ? 'All Colleges'
                                        : (colleges.firstWhereOrNull((c) => c.id == _selectedCollegeId)?.name ?? 'Select College'),
                                    style: const TextStyle(fontSize: 13),
    ...
    return Column(
      children: [
        ...
        Padding(
          ...
          child: Row(
            children: [
              const Icon(Icons.directions_bus_rounded, color: Color(0xFF1E90FF), size: 18),
              const SizedBox(width: 8),
              Text(
                'Active Global Fleet (${activeBuses.length})',
                                ...
                                children: [
                                  Column(
                                    ...
                                    children: [
                                      ...
                                      Text(
                                        colleges.firstWhereOrNull((c) => c.id == bus.collegeId)?.name ?? 'Unknown College',
                                        style: TextStyle(
                                          fontSize: 9,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildDetailRow(
              'College',
              colleges.firstWhereOrNull((c) => c.id == bus.collegeId)?.name ?? 'Unknown',
            ),
            const SizedBox(height: 8),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
            ...
            const SizedBox(height: 8),
            _buildDetailRow('Bus Number', bus.busNumber, valueColor: Colors.indigoAccent),
            const SizedBox(height: 8),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: () {
                  // Stub coordinator contact action
